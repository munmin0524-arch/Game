...
const MOCK_SESSIONS: Record<string, object> = {
  'sess-1': {
    ...
    deploy_type: 'existing_group', session_type: 'live', game_mode: 'quiz_battle',
  ...
  'sess-2': {
    ...
    deploy_type: 'existing_group', session_type: 'assignment', game_mode: 'quiz_battle',
...
export async function GET(_: Request, { params }: { params: { sessionId: string } }) {
  const session = MOCK_SESSIONS[params.sessionId] ?? MOCK_SESSIONS['sess-1']
